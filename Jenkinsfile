pipeline {
    ...
    stages {
        ...
        stage('Deploy - Stage') {
            ...
            steps {
                sh """oc set image deployment ${DEPLOYMENT_STAGE} shopping-cart-stage=quay.io/${QUAY_USR}/do400-deploying-environments:build-${BUILD_NUMBER}  -n ${APP_NAMESPACE} --record
                """
            }
        }
        stage('Deploy - Prod') {
